...
而从本章开始，我们就要开始正式进入C++的学习之旅啦~

不知道同学们知不知道，“贪吃蛇” 这款小游戏。没玩过也没关系，老师会通过游戏来介绍变量。

::: column(width=600)
    x-img(src="images/tstart.jpg" width=500 height=500)
:::

假设我们玩一个游戏，那肯定需要一个自己的昵称，还需要有最后的得分，才能知道谁得了多少分。此时在这里，我们知道，不同的**分数**和**名字**有可能是不一样的。这种变化的数据就可以当做**变量**。
...
变量，可以是贪吃蛇的长度，可以是贪吃蛇吃了食物的得分，玩家的名字，等等。

在这里，我们需要介绍一下计算机的存储空间。大家可以把它想成一个楼房，每一户都会有门牌号，每一户的家里都会住着人。把人比作数据（变量值），把门牌号比作数据的标识（变量名）。
...
在我们玩 “ 贪吃蛇 ” 的时候，是需要蛇去吃食物，来提高得分的。而得到的分数，1分，2分，3分。。。它是个整数，数学中的整数，就可以看做，数据类型中的整型；

还有，玩 “ 贪吃蛇 ” 之前，是需要我们输入昵称的，而昵称的规则是由一串字符串组成的。这就可以看做是数据类型中的字符串型；
...
- string：字符串类型（ 比如 ： camluni ）
  
- bool：布尔类型（ 它的值只有两个：真/假 ）

我们可以看到， **数据类型** 有很多种：可以是数字、可以是小数、可以是单个字母、可以使一句话，甚至可以判断是不是正确的，同学们可以玩一玩下面的动画，点击三角按钮，观察他们的变化：
...
在生活中，人的个数，楼层的数，人命币的大小，都是整数的例子。在C++中，由于计算机的存储数的长度、大小都不一样，所以整型有很多种类。而整型的默认类型是 _{code}int_ 。

##### 浮点型

在生活中，我们在网购，买东西的时候，需要花钱，有时候可能是9.9，其实它就是小数。在C++中，浮点型也叫小数。而浮点型的默认类型是 _{code}double_ ，它还有一个类型是float，但是它没有double精确，我们基本使用的就是 double 。
...
就像上面我们提到的“贪吃蛇”，它刚开始的时候，有个初始分数，为0。那么我们怎么去保存贪吃蛇的分数值呢？贪吃蛇的分数需要用什么数据类型去保存呢？
...
贪吃蛇，每吃一个食物，分数就会增加一个：0、1、2... 分数的值属于整型 _{code}int_ 。我们把贪吃蛇的分数，命名为 _{code}score_ 。那么我们可以这么声明：
...
- 在C++中，变量名只能用这三种符号：大小写字母（A、a...），数字（0、1、2...）、下划线（ _ ）。
  
- **不能**使用**数字**做变量名的**首位**。   

- 变量名区分字母**大小写**。   

- **不能**使用C++的**关键字**做变量名。（比如：int，return，double）
...
我们声明了一个分数变量后，就可以给它初始化赋值了！

此时，我们给 _{code}score_ 变量一个初始值，值为0。
...
_{code}=_ 是给变量，赋值的符号。（提示： _{code}=_ 在这里不是“等于”的意思，而是将右边的值，“赋值”给了变量，小朋友们要注意哦~）
...
在我们分别了解到声明变量以及初始化变量之后，其实它们是可以在一起用的：
...
这句话就是说，声明一个 _{code}score_ 的整型变量，并且将它的初始值设置为 _{code}0_ 。
...
**提示** ：同学们能感觉到出 4 * (4 - 2) ，和  4 * 4 - 2  他们的区别吗？  有时候，我们可以添加括号 _{code}()_ 来优先计算括号里面的程序。

##### 使用说明

_{input#arithmeticoperators(type="checkbox" disabled="disabled")}_ 1.在贪吃蛇中，初始分数为0，名为 _{code}score_ ，现在这条蛇吃了5个食物了（每次吃一个食物，分数就会加1）。它的分数现在变成多少啦？

_{input#arithmeticoperators(type="checkbox" disabled="disabled")}_ 2.我们需要在运行结果窗口输出分数值，我们可以使用 _{code}cout_ ,输出分数。
...
我们在玩 “贪吃蛇” 的游戏中。每次吃一个食物，就会增长一截，吃的越多，变得越长。这个就可以简单理解成综合输出。

假如说我们现在得了5分的分数了，上一节也展示出：5的分数值了，但是如果没有提示，我们不知道这个5分，具体代表着什么，所以我们需要，一个完整的提示。
...
经过上面的学习，我们发现，我们看到的数据，都是我们在编辑器写好了，运行展示出来的。
...
**16.** _{code} 1/2.0_ 结果是[[0.5]]。

**17.** _{code} 10 % 3_ 结果是[[1]]。

**18.** _{code} ( 2 * 2 + 2 - 1) % 2_ 结果是[[1]]。